...
drop sequence seq_user_addr;
drop sequence seq_post;
drop sequence seq_comment;
drop sequence seq_order_history;
drop sequence seq_basket;

-- 주석처리된 테이블 (tbl_order_history, table tbl_youtube, tbl_basket, tbl_discount_hist) 는 tbl_product 미존재로 인해 생성 대기.
...
create table tbl_post (
   ...
   post_cd         number not null references tbl_post_type ( post_cd ),
   user_key        char(11) not null references tbl_user ( user_key ) on delete cascade,
...
create table tbl_comment (
   ...
   post_key        char(11) not null references tbl_post ( post_key ),
   user_key        char(11) not null references tbl_user ( user_key ) on delete cascade,
   comment_content varchar2(2000) not null,
...
create table tbl_order_history (
   ...
   prod_key      char(11) not null references tbl_product ( prod_key ),
   user_key      char(11) not null references tbl_user ( user_key ) on delete cascade,
   addr_key      char(11) not null references tbl_user_addr ( addr_key ),
...
create table tbl_youtube (
   ...
   prod_key    char(11) not null references tbl_product ( prod_key ) on delete cascade
);
*/
...
create table tbl_discount_hist (
   ...
   prod_key   char(11) not null references tbl_product ( prod_key ) on delete cascade,
   event_cd   number not null references tbl_discount ( event_cd ),
   start_date date,
   end_date   date
);
*/
